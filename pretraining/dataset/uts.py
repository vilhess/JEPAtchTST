...
def create_dataloader(subset_name, window_size, batch_size=64, collator=None):
    ds = UTSDataset(subset_name=subset_name, input_len=window_size, output_len=0, flag='train', stride=1)
    dataloader = torch.utils.data.DataLoader(ds, batch_size=batch_size, shuffle=True, collate_fn=collator, num_workers=21)
    return dataloader
...
if __name__ == "__main__":
    ...
    window_size = 100
    batch_size = 64
    dataloader = create_dataloader(subset_name, window_size, batch_size)
    
    for batch in dataloader:
        print(batch.shape)  # Should print (batch_size, window_size, 1)
        break